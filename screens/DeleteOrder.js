...
export default function DeleteOrder({ navigation,route }) {
  const [ticketName,setTicketName] = useState('');
  const [user,setUser] = useState('')
  const [userId,setUserId] = useState('')
  const [role,setRole] = useState('')
  const [data,setData] = useState('');
  const [method,setMethod] = useState('Browse all orders')
...
const getSource = (name) =>{
 ...
 const getGuestOrder = async () =>{
  ...
  const deleteOrder = async (id) =>{
    const url = baseUrl+'/api/ticket/deleteOrder'
    console.log('id',id)
    await fetch(url,{
      method:'POST',
      headers:{
...
/>
    ...
    return (
      ...
      <Text style={[localStyles.header]}>Delete Order Record</Text>
      ...
      <Text>Find order by:</Text>
        <Card>
           <Picker
            ...
            <Picker.Item label="Browse all orders" value="Browse all orders" />
            <Picker.Item label="Passport" value="Passport" />
            </Picker>
        </Card>

        {method=='Browse all orders'?content:passportSearch}
